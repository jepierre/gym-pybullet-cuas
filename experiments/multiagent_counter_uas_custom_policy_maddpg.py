...
from ray.rllib.contrib.maddpg import MADDPGTrainer, DEFAULT_CONFIG
...
if __name__ == "__main__":
    ...
    config["multiagent"] = {
        ...
        "policies": {
            ...
            "pol0": (ApfPolicy, observer_space, action_space, {}),
    ...
    results = tune.run(
        ...
        # restore = r'/home/marcus/Documents/workspace/reinforcement-learning/pybullet/gym-pybullet-drones/experiments/learning/results/save-leaderfollower-4-cc-kin-vel-06.19.2021_11.38.25/PPO/PPO_this-aviary-v0_29260_00000_0_2021-06-19_11-38-30/checkpoint_000646/checkpoint-646',
        # resume =  True
    )
    ...
    for i in range(100 * int(test_env.SIM_FREQ / test_env.AGGR_PHY_STEPS)):  # Up to 6''
        ...
        for i in range(2):
            ...
            temp[i] = policy0.compute_actions(
                [
                    np.hstack(
                        [
                            action[opponent_idx],
